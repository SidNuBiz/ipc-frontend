...
const AdminWhoWeArePageContentEdit = () => {
    ...
    const [mambersArr,setMembersArr] = useState([])
    ...
    newDesignationsArr.push({id:uuidv4(),point:designations})
    setDesignations('')
    ...
    editedDesignationsArr[idx].point = value
    setDesignationsArr(editedDesignationsArr)
    }

    const deleteDesignations = (value) => {
    setDesignationsArr(
        designationsArr.filter((item) => item.id !== value)
    );
    }

    const [memberImage,setMemberImage] = useState()
    const [previewMemberImage,setPreviewMemberImage] = useState(noImg)
                            ...
                            <tbody>
                                ...
                                designationsArr.map((item,index) => {
                                    ...
                                    <tr key={index} className=''>
                                        ...
                                        <td>
                                        <input type="text" className='mr-2 w-full bg-transparent mt-5 px-5 py-3 border-gray-300 border-[1px] focus:outline-none' onChange={(e)=>editDesignations(index,e.target.value)}  defaultValue={item} required />
                                        </td>

                                        <td className='h-full'>
                                        <button onClick={() => deleteDesignations(index)} className="text-white rounded-lg hover:scale-125 duration-300 h-full w-full"><img src="https://img.icons8.com/windows/35/c70000/trash.png" alt="" className='h-[28px] w-[28px] m-2'/></button>
                                        </td>
                        ...
                        <div >
                            ...
                            <table className='w-full pt-1 mt-1 '>
                                ...
                                <tr className=' text-gray-600 font-semibold'>
                                    ...
                                    <input id='new-service-point' type="text" className=' mr-2 w-full bg-white mt-5 px-5 py-3 border-gray-300 border-[1px] focus:outline-none' onChange={(e) => {setDesignationsArr(e.target.value)}} value={designations} placeholder='Designation' required />
